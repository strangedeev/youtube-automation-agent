...
class AnalyticsOptimizationAgent {
  ...
  async analyzeVideoPerformance(videoId) {
    try {
      ...
      // Get analytics data — skip the video entirely when real data is
      // unavailable (new video, quota error). Never analyze fabricated numbers.
      const analytics = await this.getVideoAnalytics(videoId);
      if (!analytics) {
        this.logger.warn(`No real analytics available for ${videoId} — skipping analysis`);
        return null;
      }
  ...
  async getVideoAnalytics(videoId) {
    ...
    try {
      ...
      // Return null rather than fabricated numbers — downstream consumers must
      // skip missing data, never optimize against Math.random().
  ...
  async getDemographicsAnalytics(videoId, startDate, endDate) {
    try {
      ...
      // No fabricated demographics — absent data is reported as null fields.
      return { ageGroups: [], gender: [], primaryAudience: null };
    }
  }
