...
class Database {
  ...
  async createTables() {
    ...
    // Migration: publish_schedule didn't originally track which content angle
    // (facts/story/mystery/betrayal/truecrime) or format (short/long) a video
    // came from, so performance could never be attributed back to a niche.
    // ALTER TABLE has no "IF NOT EXISTS" in SQLite — ignore the error if the
    // column is already there from a previous run.
    for (const col of ['angle TEXT', 'format TEXT']) {
      try { await this.executeQuery(`ALTER TABLE publish_schedule ADD COLUMN ${col}`); } catch (_) {}
    }
  ...
  async saveScheduleEntry(entry) {
    ...
    await this.executeQuery(
      `INSERT INTO publish_schedule (
        ...
        priority, metadata, angle, format
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        entry.productionId,
